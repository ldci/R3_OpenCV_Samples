...
opencv?: no				;--is opencv extension used?
...
generatePoints: function [nbPoints [integer!] radius [float!]][
...
showPoints: function [points [block!]][
	im: make image! reduce [3x3 black]		;--for Red
    ...
    foreach v points [   
    	...
    	im/rgb: color						;--update image (Red)
    	change at image1 p im				;--update image (Red)
		;change/dup at image1 p color 3x3	;--update image (Rebol)
	]
]
...
lloyd: function [points [block!] nCluster [integer!]][
	...
	until [
		...
		repeat j len [
			...
			c: clusters/(p/group)	;--clone centroid object according to point index
...
showKmeans: function [points [block!] bclusters [block!] nCluster [integer!]][
    ...
    repeat i nCluster [
    	repeat j len [
    		...
    		im/rgb: color						;--Red
    		change at image2 p im				;--Red
    		;change/dup at image2 p color 3x3	;--show generated points in image2 (Rebol)
		]
    ]
    im: make image! reduce [6x6 red]
    repeat i nCluster [
    	...
    	im/rgb: red								;--Red
    	change at image2 p im					;--Red
    	;change/dup at image2 p red 6x6			;--show centroid (mean) Rebol
    ]
]
